...
class WritableBuffer extends LiberalBuffer {
  ...
  reserve(length, callback, value) {
    ...
    let chain = that.chain || (that.chain = _pull(that, length));
    ...
    if (callback) {
      return callback(chain.buffer, offset, value);
    }
  }

  // shortcut
  writeUInt8(value) {
    return this.reserve(1, _writeUInt8, value);
  }
...
function _writeUInt8(buffer, offset, value) {
