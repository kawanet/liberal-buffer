...
class WritableBuffer extends LiberalBuffer {
  ...
  reserve(length, value, callback) {
    ...
    return callback && callback(chain.buffer, offset, value);
  }

  writeString(value) {
    const tempLength = value.length * 3;
    const realLength = this.reserve(tempLength, value, _writeString);
  ...
  writeBuffer(value, start, end) {
    ...
    return this.reserve(length, tmp, _writeBuffer);
  }
...
((P) => {
  ...
  function wrap(method, length) {
    return function(value) {
      this.reserve(length, value, write);
      return length;
    };
