...
class ReadableBuffer extends LiberalBuffer {
  reserve(length, callback) {
    ...
    if (callback) return callback(chain.buffer, prevOffset, nextOffset);
  }
...
function _reserve(that, length) {
  ...
  const latter = former.next || (former.next = LiberalBuffer(_pull(that, length)));
...
function _pull(that, length) {
  ...
  if (!buffer) throw new Error("BUFFER_SHORTAGE");
}
